...
HANDLE dlOpenWithCheck(const char *libraryPath)
{
   ...
   if (handle < (HANDLE)32)
   {
      printf("Error in LoadLibrary. Error code: %zu\n", (size_t)handle);
      return NULL;
   }
#else
   if (!handle)
...
{
   HANDLE indigoHandle = NULL;
   HANDLE indigoInChIHandle = NULL;
   HANDLE indigoRendererHandle = NULL;
   HANDLE bingoHandle = NULL;
   ...
   const char *indigoLibraryPath = NULL;
   const char *indigoInChILibraryPath = NULL;
   const char *indigoRendererLibraryPath = NULL;
   const char *bingoLibraryPath = NULL;
   ...
   if (indigoInChITest)
   {
      ...
      printf("IndigoInChI address: %zu\n", (size_t)indigoInChIHandle);
   ...
   if (indigoRendererTest)
   {
      ...
      printf("IndigoRenderer address: %zu\n", (size_t)indigoRendererHandle);
      indigoLoadMoleculeFromString = (INT_RET_STR)DLSYM(indigoHandle, "indigoLoadMoleculeFromString");
